...
var ConfigSchema = import_zod.z.object({
  publicPath: import_zod.z.string().optional().describe("The public path to the wasm files and the onnx model.").default(`https://unpkg.com/${package_default.name}@${package_default.version}/dist/`),
  ...
  progress: import_zod.z.function().args(import_zod.z.string(), import_zod.z.number(), import_zod.z.number()).returns(import_zod.z.undefined()).describe("Progress callback.").optional(),
  model: import_zod.z.enum(["small", "medium"]).default("medium"),
  output: import_zod.z.object({
    format: import_zod.z.enum(["image/png", "image/jpeg", "image/webp", "image/x-rgba8"]).default("image/png"),
    quality: import_zod.z.number().default(0.8),
    type: import_zod.z.enum(["foreground", "background", "mask"]).default("foreground")
  }).default({})
}).default({});
...
async function runInference(imageTensor, config, session) {
  ...
  if (config.output.type === "mask") {
    tensorImage = (0, import_ndarray4.default)(new Uint8Array(4 * stride), [
      resolution,
      resolution,
      4
    ]);
